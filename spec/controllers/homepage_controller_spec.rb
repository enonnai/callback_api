...
RSpec.describe HomepageController do
  ...
  describe "POST create" do
    ...
    context "when the API DOES NOT raise an exception after receiving a POST request" do
      before(:each) do
    ...
    context "when the API raises an exception after receiving a POST request" do
      before(:each) do
    ...
    context "when the API returns an Internal Server Error POST request" do
      before(:each) do
        allow(SecureRandom).to receive(:uuid).and_return("189a607d-9950-4f8c-845e-20d4b0b4b3f4")
        stub_request(:post, "http://mic-leads.dev-test.makeiteasy.com/api/v1/create").
           with(
